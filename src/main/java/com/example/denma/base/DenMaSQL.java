...
public class DenMaSQL implements Serializable{
    ...
    public static Assistant getAssistantUser(String user){
        DenMaSQL dmsql=getDBInformation();
        ...
        try {
            ...
            while(rss.next())
            {
                if (rss.getString("userName").equals(user)){
                    // patient.add(newPatient(rss.getInt("idPatient"),rss.getDate("dateNaissance").toLocalDate(),rss.getString("cin")
                    //         ,rss.getString("nom" ),rss.getString("prenom"),rss.getString("sexe").charAt(0),new CouvertureMedicale(rss.getString("idCouverture"),rss.getString("typeCouverture"))));
                    assistant = new Assistant(rss.getInt("IDAssistant"),rss.getDate("DateNaissance").toLocalDate(),
                            rss.getString("CIN"),rss.getString("Nom" ),rss.getString("Prenom"),
                            rss.getString("password"), rss.getString("userName"),rss.getString("Sexe").charAt(0));
                }
            }
        }
        catch (Exception e){
