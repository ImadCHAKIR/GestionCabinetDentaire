...
public class DentistMenuController implements Initializable {
    ...
    @FXML
    private AnchorPane ParaSecurity;
    ...
    @FXML
    private JFXButton DeleteAssistant;

    @FXML
    private JFXButton EditAssistant;

    @FXML
    private JFXButton ajouterNouveauAssistant;
    ...
    public void initialize(URL location, ResourceBundle resources){
        try{
            ...
            ajouterNouveauAssistant.setOnAction(new EventHandler<ActionEvent>() {
                @Override
                public void handle(ActionEvent actionEvent) {
                    addAssistant();
                }
            ...
            Assistant ass = DenMaSQL.getAssistantUser(user.getLogin());

            EditAssistant.setOnAction(new EventHandler<ActionEvent>() {
                public void handle(ActionEvent event) {
                    try {
                        AddAssistantController aac = new AddAssistantController(ass);
                        aac.showStage();
                    } catch (Exception e) {
                        AlertBox alert = new AlertBox(AlertBox.AlertBoxType.ERROR);
                        alert.setTitleAndHeader("Error",e.getMessage());
    ...
    public void addAssistant(){
        new AddAssistantController(this).showStage();
    }
    ...
    public void interventionStatsInit() {
        String[] Colors ={"#ffd700","#ffa500","#ff00b7","#2bff00","#00ffff","#3700ff","#520086"};
        ...
        for (int i=0;i< ntr.size();i++) {
            interventionsStats.getData().add(new PieChart.Data(ntr.get(i), stats[i]));
            Tooltip tph = new Tooltip(ntr.get(i)+" : " +stats[i]);
            tph.setStyle("-fx-background-color:"+Colors[i]+";");

            tph.setShowDelay(Duration.seconds(0.1));
            tph.setShowDuration(Duration.seconds(20));
            Tooltip.install(interventionsStats.getData().get(i).getNode(), tph);
        }
